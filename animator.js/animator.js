...
var CubicBezierGenerator = function CubicBezierGenerator(X1, Y1, X2, Y2) {
  ...
  return function /*BezierEasing*/ (/*x=0*/) {
    ...
    if (x < 0 || 1 < x) {
      console.warn("CubicBezierGeneratorError: Time parameter exceeds range [0,1]. Values will be clipped.");
      x = x < 0 ? 0 : 1 < x ? 1 : 0;
    } else if (0 === x || 1 === x) {
      return x;
    }
...
HTMLElement.prototype.animator = {
  ...
  _internal$: {
    ...
    play: function play() {
      ...
      function executeFrame(now_i) {
        now = now_i ? now_i + TIMEORIGIN : Date.now();
        ...
        if (now - this._internal$._sub$.beganAt <= this.duration * 1000 && this._internal$._sub$.playing) {
          ...
          (requestAnimationFrame || window.requestAnimationFrame || setInterval || window.setInterval)(executeFrame.bind(this));
  ...
  // readonly properties on object root-level from frozen internals
  get iteration() {
    return this._internal$._sub$.iteration;
  },
  get pause() {
    return this._internal$.pause.bind(this);
  },
  get play() {
    return this._internal$.play.bind(this);
  },
  get reset() {
    return this._internal$.reset.bind(this);
  },
  get stop() {
    return this._internal$.stop.bind(this);
  }
};
Object.freeze(HTMLElement.prototype.animator._internal$);
